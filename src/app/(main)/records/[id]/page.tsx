...
import type { DiscogsCommunity } from '@/types/discogs';
...
export default function RecordDetailPage({
  ...
  const [discogsRating, setDiscogsRating] = useState<{ average: number; count: number } | null>(null);

  useEffect(() => {
    fetchRecord();
  }, [resolvedParams.id]);

  // Fetch Discogs rating when record loads
  useEffect(() => {
    if (record?.discogsId) {
      fetch(`/api/discogs/release/${record.discogsId}`)
        .then((res) => res.json())
        .then((data) => {
          if (data.community?.rating) {
            setDiscogsRating(data.community.rating);
          }
        })
        .catch((err) => console.error('Failed to fetch Discogs rating:', err));
    }
  }, [record?.discogsId]);
        ...
        <div className="space-y-4 sm:space-y-6">
          ...
          <Card variant="bordered" className="p-3 sm:p-4">
            <div className="grid grid-cols-2 gap-3 sm:gap-4">
              ...
              {discogsRating && discogsRating.average > 0 && (
                <div>
                  <p className="text-xs sm:text-sm text-vinyl-400">Discogs Rating</p>
                  <div className="flex items-center gap-1.5">
                    <div className="flex items-center gap-0.5">
                      {[1, 2, 3, 4, 5].map((star) => (
                        <svg
                          key={star}
                          className={`w-4 h-4 ${
                            star <= Math.round(discogsRating.average)
                              ? 'text-accent-gold'
                              : 'text-vinyl-600'
                          }`}
                          fill="currentColor"
                    ...
                    <span className="text-sm sm:text-base text-vinyl-100 font-medium">
                      {discogsRating.average.toFixed(1)}
                    </span>
                    <span className="text-xs sm:text-sm text-vinyl-400">
                      ({discogsRating.count.toLocaleString()})
                    </span>
                  </div>
                </div>
              )}
