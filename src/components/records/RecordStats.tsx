        ...
        {stats.map((stat) => (
          <Card key={stat.label} variant="elevated">
            <div className="flex items-center gap-4">
              <div className="p-3 rounded-lg bg-accent-purple/20 text-accent-purple">
                {stat.icon}
              </div>
              <div>
                <p className="text-3xl font-bold text-vinyl-50">{stat.value}</p>
                <p className="text-sm text-vinyl-400">{stat.label}</p>
              </div>
            </div>
          </Card>
        ))}
      ...
      {latestRecord && (
        <Card variant="bordered" className="mt-4">
          <div className="flex items-center gap-3">
            ...
            <p className="text-sm text-vinyl-300">
              Latest addition:{' '}
              <span className="font-medium text-vinyl-100">
                {latestRecord.title}
              </span>{' '}
              by {latestRecord.artist}
            </p>
          </div>
        </Card>
      )}
